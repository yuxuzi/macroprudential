...
(function(global, factory) {
   ...
   * @param startWidth [number] "Starting width of zoomer image"
   */
  var options = {
    onZoomCallback: $.noop,
    controls: {
  ...
  var properties = {
    ...
    // Timer to measure how long the mouse is held down. When downTimer > maxDownTimer, we zoom continuously
    downTimer: 0,
    maxDownTimer: 500,

    // Set increments to zoom in or out to when the zoom buttons are pressed
    zoomIncrements: [0.16, 0.25, 0.33, 0.5, 0.66, 1, 2, 3, 4, 5, 7, 8, 12, 16],
  };
  ...
  var pub = {
     ...
     * @method
     * @name zoomScroll
     * @description Zoom in or out based on a certain delta
     * @example $(".target").zoomer("zoomScroll", delta);
     */
    zoomScroll: function(delta) {
      return $(this).each(function(i, target) {
        var data = $(target).data("zoomer");
        ...
        if (data) {
          ...
          data.keyDownTime = 1;
          data.action = "zoom_scroll";
          _updateValues(data, delta);
     ...
     * @example $(".target").zoomer("zoom100", 50, 50);
     */
    zoom100: function(left, top) {
      return $(this).each(function(i, target) {
        var data = $(target).data("zoomer");

        if (data) {
          data = _setZoomPosition(data, left, top);
          data.keyDownTime = 1;
          data.action = "zoom100";
  ...
  function _build($target, data) {
    if (!$target.data("zoomer")) {
      ...
      if (data.controls.zoomIn || data.controls.zoomOut || data.controls.zoomLabel || data.controls.next || data.controls.previous) {
        ...
        data.controls.$zoomLabelContainer = $(data.controls.zoomLabelContainer);
      ...
      // Bind events
      data.controls.$zoomIn.on("touchstart.zoomer mousedown.zoomer", data, _onZoomButtonDown)
        .on("touchend.zoomer mouseup.zoomer", data, _onZoomButtonUp);
      data.controls.$zoomOut.on("touchstart.zoomer mousedown.zoomer", data, _onZoomButtonDown)
        .on("touchend.zoomer mouseup.zoomer", data, _onZoomButtonUp);
      // Needed for double click to zoom out without delay
      data.controls.$zoomOut.on("zoomout", data, _zoomOut);

      data.controls.$zoomLabelContainer.on("touchstart.zoomer mousedown.zoomer", data, _zoomLabel)
  ...
  function _onImageLoad(e) {
    ...
    if (data.startWidth > 0) {
      data.targetImageWidth = data.startWidth;
      data.targetImageHeight = data.imageRatioTall * data.targetImageWidth;
    } else {
    ...
    // Use only the increments that are large enough.
    var index = 0;
    while (data.naturalWidth * data.zoomIncrements[index] < data.minWidth) {
      index++;
    }
    data.zoomIncrements = data.zoomIncrements.slice(index, data.zoomIncrements.length);
    ...
    // We want loading to happen instantly
    data.$holder.css({opacity: 1});
    data.loading = false;
  ...
  function _render() {
    for (var i = 0, count = $instances.length; i < count; i++) {
      ...
      if (typeof data === "object") {
        ...
        // Update zoom label
        var zoomLevel = Math.round(scaleX*100) || 100;
   ...
   * @param data [object] "Instance Data"
   */
  function _updateValues(data, delta) {
    // Update values based on current action
    if (data.action === "zoom_in" || data.action === "zoom_out" || data.action === "zoom_scroll") {
      // Calculate change
      data.keyDownTime += data.increment;
      if (delta === undefined)
        delta = ((data.action === "zoom_out") ? -1 : 1) * Math.round((data.imageWidth * data.keyDownTime) - data.imageWidth);
    ...
    if (data.action === "zoom_in_incremented") {
      var index = 0;
      // We use 3px for margin of error because zooming is calculated continuosly and is rounded
      while (index < data.zoomIncrements.length && data.zoomIncrements[index] * data.naturalWidth <= data.imageWidth + 3) {
        index++;
      }
      if (index === data.zoomIncrements.length) {
        data.targetImageWidth = data.maxWidth;
        data.targetImageHeight = data.maxHeight;
      } else {
        data.targetImageWidth = data.zoomIncrements[index] * data.naturalWidth;
        data.targetImageHeight = data.zoomIncrements[index] * data.naturalHeight;
      }
    } else if (data.action === "zoom_out_incremented") {
      var index = data.zoomIncrements.length - 1;
      while (index >= 0 && data.zoomIncrements[index] * data.naturalWidth >= data.imageWidth - 3) {
        index--;
      }
      if (index < 0) {
        data.targetImageWidth = data.minWidth;
  ...
  /**
   * @method private
   * @name _onZoomButtonDown
   * @description Handles when zoom button is pressed
   * @param e [object] "Event data"
   */
  function _onZoomButtonDown(e) {
    e.preventDefault();
    e.stopPropagation();

    var data = e.data;
    var up = $(this).hasClass("zoom-in");
    data = _setZoomPosition(data);
    if (up) {
      data.action = "zoom_in_incremented";
    } else {
      data.action = "zoom_out_incremented";
    }
  ...
  /**
   * @method private
   * @name _onZoomButtonUp
   * @description Handles when zoom button is released
   * @param e [object] "Event data"
   */
  function _onZoomButtonUp(e) {
    if (e.preventDefault) {
      e.preventDefault();
      e.stopPropagation();
    }
    clearTimeout(e.data.downTimer);
    _clearZoom(e);
  }
